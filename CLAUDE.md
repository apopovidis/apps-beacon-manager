...
- **It is not currently reachable, but the root cause is now confirmed from real Railway deploy logs** (the earlier IPv6-reachability guess was wrong — disregard it): `ANALYTICS_DATABASE_URL` on Railway is a Supabase connection-pooler string with the wrong region in the hostname, so Supabase's pooler rejects every connection with `(ENOTFOUND) tenant/user postgres.uccbvarrihdtsjingmpu not found` — a real rejection from the pooler, not a network failure. That also exposed a real bug in `../apps-analytics-sdk/src/db/pool.ts` (now fixed, commit `787981b`): `pg.Pool` emits its own `'error'` event for idle-client connection failures, and with no listener registered, Node treated that as an uncaught exception and crashed the whole process — which is why even `/health` (no DB touch) started 502ing, not just the DB-touching routes.
- **Next step**: get the exact pooler connection string from Supabase's dashboard (don't hand-type the region), update Railway's `ANALYTICS_DATABASE_URL`, redeploy, confirm `/health` and `/admin/projects` both respond.
